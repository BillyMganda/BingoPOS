...
{
    public partial class Reprint_By_Card_Number : Form
    {
        ReportDocument crystal = new ReportDocument();
        public Reprint_By_Card_Number()
        {
            InitializeComponent();
        ...
        //LOAD
        private void Reprint_By_Card_Number_Load(object sender, EventArgs e)
        {
            label1.Text = Reprint_Validator.Card_Number;
            label2.Text = Reprint_Validator.Selected_Date;
            label3.Text = Reprint_Validator.Selected_Time;
            try
            {
                SqlConnection con = new SqlConnection(connectionstring);
                SqlDataAdapter sda = new SqlDataAdapter("SELECT * FROM Sales WHERE CardNumber = '"+label1.Text+"' AND SessionDate = '"+Convert.ToDateTime(label2.Text)+"' AND SessionTime = '"+Convert.ToDateTime(label3.Text)+"' ", con);
